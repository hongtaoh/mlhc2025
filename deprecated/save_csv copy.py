...
def main():
    ...
    # ml_order or order_with_highest_ll
    which_tau_better = defaultdict(lambda: defaultdict(int))
    which_mae_better = defaultdict(lambda: defaultdict(int))
        ...
        for fname in tqdm(files, desc=f"{algo}", leave=False):
            ...
            try:
                ...
                if algo in SA_EBM_ALGO_NAMES:
                    ...
                    ml_stages_expected = [
                        int(math.ceil(np.sum(np.arange(1, len(final_stage_post[str(pid)]) + 1) * final_stage_post[str(pid)])))
                    ...
                    mae_expected = mean_absolute_error(true_stages, ml_stages_expected)
                    mae_soft = mean_absolute_error(true_stages, ml_stages_soft)
                    if mae_soft < data['mean_absolute_error']:
                        which_mae_better[algo]['mae_soft_lower'] += 1
                    elif mae_soft > data['mean_absolute_error']:
                        which_mae_better[algo]['mae_max_lower_than_soft'] += 1

                    if mae_expected < data['mean_absolute_error']:
                        which_mae_better[algo]['mae_expected_lower'] += 1
                    elif mae_expected > data['mean_absolute_error']:
                        which_mae_better[algo]['mae_max_lower_than_expected'] += 1

                    if mae_expected > mae_soft:
                        which_mae_better[algo]['mae_soft_lower_than_expected'] += 1
                    elif mae_expected < mae_soft:
                        which_mae_better[algo]['mae_expected_lower_than_soft'] += 1
                    ...
                    qwk_result = data['quadratic_weighted_kappa']
    ...
    if which_mae_better:
        sorted_mae_data = {
            k: dict(sorted(v.items()))
            for k, v in which_mae_better.items()
        }
        with open ('which_mae_better.json', 'w') as f:
            json.dump(sorted_mae_data, f, indent=4)
